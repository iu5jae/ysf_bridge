...
def aprs_string(call, latitude, longitude, radio_code, ysf_code):
  global APRS_LH
  radio_str = ''
  table = '/'
  symb = '['
...
# - -> Home
  ...
  if ((len(call) < 3) or ((latitude == 0.0) and (longitude == 0.0))):
    return ''
...
def send_aprs(aprs_string, aprs_server, aprs_user):  
  ...
  try:
    ...
    AIS = aprslib.IS(aprs_user, host=aprs_server, passwd=passw, port=14580)
    AIS.connect()
